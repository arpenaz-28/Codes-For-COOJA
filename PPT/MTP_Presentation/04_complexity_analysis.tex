...
\begin{frame}{Formal Security Analysis -- ProVerif}
    \begin{columns}
        \begin{column}{0.42\textwidth}
            \scriptsize
            Modeled under the \textbf{Dolev--Yao} adversary (full control of the open channel).
            \textbf{All queries satisfied:}
            \begin{itemize}
                \item \textbf{Correspondence:} every step runs in order and cannot be forged
                $\Rightarrow$ stops \textbf{impersonation} and \textbf{replay}.
                \item \textbf{Secrecy:} session key $K_{GW\text{-}D}$ never derivable
                $\Rightarrow$ an eavesdropper \textbf{cannot read data}.
                \item \textbf{Forward secrecy:} $m_{\mathrm{new}}$ stays secret $\Rightarrow$ a leaked
                key \textbf{exposes no past/future sessions}.
                \item \textbf{Anonymity:} $ID_D$ unrecoverable $\Rightarrow$ device
                \textbf{cannot be tracked}.
                \item \textbf{Offline guessing:} key not brute-forceable from captured traffic
                $\Rightarrow$ \textbf{dictionary attacks fail}.
            \end{itemize}
        \end{column}
    ...
    \begin{columns}
        \begin{column}{0.42\textwidth}
            ...
            \begin{itemize}
                ...
                \item LAAKA / Zhou grow steeply (larger payloads, more message exchanges).
