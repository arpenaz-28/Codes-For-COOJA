/* ==========================================================================
 * device-node.c  —  IoT Device Node  (Revised Anonymity Scheme — TWO-ROUND)
 *
 * Auth and Key Exchange are SEPARATE CoAP rounds so the protocol is
 * explicitly two-round and comparable to standard schemes.
 *
 * State machine:
 ...
 *
 * Measurement methodology mirrors base scheme exactly (same variable names,
 * same snapshot positions) for direct energy comparison.
 * ========================================================================== */
...
static uint8_t auth_PID[32];   /* PID sent in Round 1, reused in Round 2 */
...
/* --------------------------------------------------------------------------
 * Energest — identical variable names to base scheme for direct comparison
 * -------------------------------------------------------------------------- */
...
/* Combined auth+keyex window (mirrors base scheme's cpu_reg / cpu_auth pair) */
 ...
 * Device derives m_new → K_GW_D → new PID.
 * -------------------------------------------------------------------------- */
static void client_keyex_handler(coap_message_t *resp)
{
    ...
    /* Recover m_new from m_H (same computation as original single-round scheme) */
    uint8_t R_d = regenerate_response(c_d, h_d);
    uint8_t Y_dH[32];
    memcpy(Y_dH, auth_Y_dH, 32);

    /* mH_mask = H(Y_dH || m_d || R_d || ID_AS || auth_PID || ts_2) */
 ...
 * Main process
 * ========================================================================== */
PROCESS(device_node, "Device Node (Revised Anonymity — Two-Round)");
...
PROCESS_THREAD(device_node, ev, data)
{
    ...
    /* Round-robin across NUM_ACTIVE_AS active AS nodes starting at AS_NODE_BASE */
    id_as = (uint8_t)(AS_NODE_BASE + ((node_id - FIRST_DEVICE_ID) % NUM_ACTIVE_AS));
    ...
    while (1) {
        ...
        if (etimer_expired(&et)) {
             ...
             *
             * BEFORE snapshot taken here — mirrors base scheme's cpu_reg pos.
             * ================================================================ */
             ...
             * Expects: m_H(32)            = 32 B
             *
             * AS forwards GW token in this handler (after Round 2), so GW
             * always has the session key BEFORE device sends data.
             *
             * AFTER snapshot taken here — mirrors base scheme's cpu_auth pos.
             * ================================================================ */
                ...
                uint8_t pk[33];
                memcpy(pk, auth_PID, 32);   /* same PID as Round 1 */
                pk[32] = ts_2_last;          /* echo ts_2 to prove Round 1 receipt */
             ...
             * Session key k_gw_d and new PID are set; GW already has token.
             * ================================================================ */
