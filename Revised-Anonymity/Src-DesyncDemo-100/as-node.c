/* ==========================================================================
 * as-node.c  —  Authentication Server (Desync Demonstration, 100-node)
 *
 * Adapted for the 100-node topology where device IDs are 81–100.
 * Client array is indexed by (id_d - FIRST_DEVICE_ID) instead of id_d.
 *
 ...
 * Indexed by (id_d - FIRST_DEVICE_ID), so indices 0..NUM_DEVICES-1.
 * -------------------------------------------------------------------------- */
typedef struct {
    uint8_t  ID_d;            /* actual device node ID (81..100)       */
    uint8_t  c_d;
    uint8_t  c_as_d;
    uint8_t  phi_as_d;
...
/* Validate device ID and return array index, or -1 if invalid. */
static int client_idx(uint8_t id_d)
{
    if (id_d < FIRST_DEVICE_ID || id_d >= FIRST_DEVICE_ID + MAX_CLIENTS)
        return -1;
    return (int)(id_d - FIRST_DEVICE_ID);
}
                            ...
                            uint8_t *buf, uint16_t ps, int32_t *off)
{
    ...
    int idx = client_idx(id_d);
    if (idx < 0) return;

    clients[idx].ID_d          = id_d;
    clients[idx].enrolled      = 0;
    clients[idx].pid_old_valid = 0;
    clients[idx].last_ts1      = 0;

    clients[idx].c_d = (uint8_t)(random_rand() & 0xFF);
    gen_random(clients[idx].m_curr, 32);
    memcpy(clients[idx].m_old, clients[idx].m_curr, 32);

    uint8_t reply[REG0_REP_LEN];
    memset(reply, 0, REG0_REP_LEN);
    reply[0] = clients[idx].c_d;
    memcpy(reply + 1, clients[idx].m_curr, 32);
    aes_enc(K_AS_D, reply, 3);

    coap_set_payload(resp, reply, REG0_REP_LEN);
    printf("DESYNC_LOG|AS %u|Reg-0 for device %u|c_d=%u\n",
           node_id, id_d, clients[idx].c_d);
}
                             ...
                             uint8_t *buf, uint16_t ps, int32_t *off)
{
    ...
    uint8_t id_d = plain[0];
    int idx = client_idx(id_d);
    if (idx < 0 || clients[idx].ID_d != id_d) return;
    ...
    clients[idx].c_as_d = c_as_d;

    for (int i = 0; i < 32; i++) T_acc[i] &= Y_dH[i];

    uint8_t R_as = puf_response(c_as_d);
    clients[idx].phi_as_d = R_as ^ R_d;

    /* PID_curr = H(id_d || m_curr) — uses actual device ID */
    uint8_t pid_in[33];
    pid_in[0] = id_d;
    memcpy(pid_in + 1, clients[idx].m_curr, 32);
    H(pid_in, 33, clients[idx].PID_curr);

    memset(clients[idx].PID_old, 0, 32);
    clients[idx].pid_old_valid = 0;
    clients[idx].enrolled = 1;
    ...
    printf("DESYNC_LOG|AS %u|Reg-1 complete for device %u|PID_curr=%02x%02x%02x|pid_old_valid=0\n",
           node_id, id_d,
           clients[idx].PID_curr[0], clients[idx].PID_curr[1], clients[idx].PID_curr[2]);
}
                             ...
                             uint8_t *buf, uint16_t ps, int32_t *off)
{
    ...
    if (use_old) {
        printf("DESYNC_LOG|AS %u|RECOVERY|Using m_old=%02x%02x%02x... for device %u\n",
               node_id, cl->m_old[0], cl->m_old[1], cl->m_old[2], cl->ID_d);
    } else {
        printf("DESYNC_LOG|AS %u|Using m_curr=%02x%02x%02x... for device %u\n",
               node_id, cl->m_curr[0], cl->m_curr[1], cl->m_curr[2], cl->ID_d);
    }
    ...
    uint8_t enc_tok[48];
    memset(enc_tok, 0, 48);
    enc_tok[0] = cl->ID_d;          /* actual device ID (81..100) */
    ...
    /* PID_new = H(id_d || m_new) — use actual device ID, same as device computes */
    uint8_t new_pid_in[33];
    new_pid_in[0] = cl->ID_d;
...
PROCESS_THREAD(as_proc, ev, data)
{
    ...
    printf("DESYNC_LOG|AS %u|Started (100-node topology, devices %u..%u)\n",
           node_id, FIRST_DEVICE_ID, FIRST_DEVICE_ID + NUM_DEVICES - 1);
