...
in per-device energy over LAAKA~\cite{ali2024laaka} and 32.4\,\% over
Zhou~\cite{zhou2024auth}, with consistent gains across all network sizes and
authentication server pool configurations.
Hardware evaluation on Raspberry Pi~4B confirms
52.5\,\% and 14.2\,\% energy reductions over LAAKA and Zhou, respectively;
DAuth~\cite{das2026comsnets} achieves the lowest hardware overhead (0.152\,J) as
it carries no pseudonym operations, while the proposed scheme (0.286\,J) remains
well below Zhou (0.333\,J) and LAAKA (0.602\,J).
...
\begin{enumerate}
  \item An improved scheme over Das~\cite{das2026comsnets} is proposed, adding
    ...
    desynchronisation recovery mechanism. The design instantiates a modified form
    of the GF2 general framework of~\cite{zhong2025survey} — the only framework in
    that taxonomy to jointly provide anonymity, forward secrecy, and
    desynchronisation resistance — re-realised with lightweight PUF and hash-XOR
    primitives.
...
Concretely, the proposed scheme composes the user-anonymity sub-frameworks
(SF3-UA, SF5-UA, SF6-UA), which prescribe rotating pseudonyms in place of a static
identity, with the forward-secrecy sub-frameworks (SF7-FS, SF8-FS, SF9-FS), which
require every session key to be derived from independently refreshed secret material.
Departing from the survey's reference constructions — which rely on comparatively
heavy cryptographic building blocks — the proposed scheme re-instantiates these
sub-frameworks using only lightweight PUF-based secret derivation and hash-XOR
masking, and satisfies GF2's desynchronisation-resistance requirement through a
concrete dual-state recovery mechanism maintained at both the authentication server
and the gateway.
The result is a GF2-compliant design achieved at the low computation and
communication overhead characteristic of the base DAuth scheme~\cite{das2026comsnets}.
...
dual-state lookup, and pseudonym rotation — the computational price of anonymity and
...
\begin{enumerate}
  ...
  \item \textbf{Network scalability study:} Total node count varied across
    $N \in \{30, 50, 80, 100, 120\}$, with 20\,\% of nodes as newly joined devices.
\end{enumerate}
...
For Zhou et al., the reported costs comprise the enrolment and authentication phases
only, as that scheme folds session-key agreement into its authentication exchange
instead of executing a separate key-exchange round.
...
Zhou et al.\ has no authentication server pool and its cost remains essentially
unchanged across pool sizes, confirming that a centralised design cannot convert
authenticator diversity into load relief.
...
because their heavier per-device cryptographic load — AES-intensive authentication
in LAAKA and a combined heavy authentication and key exchange in Zhou et al.\ —
accumulates faster as the device count grows.
...
The proposed scheme consumes \textbf{0.286\,J} per round — an overhead of
approximately 88\,\% over DAuth attributable to the additional SHA-256 evaluations
for pseudonym rotation and dual-state verification.
...
Together, these two mechanisms make the proposed scheme a lightweight instantiation
of the \textbf{GF2} general framework of~\cite{zhong2025survey} — the only framework
in that taxonomy to jointly satisfy anonymity, forward secrecy, and desynchronisation
resistance — re-realised here using only PUF and hash-XOR primitives.
...
\begin{itemize}
  \item The proposed scheme achieves a per-device three-phase energy of 52.19\,mJ,
    reducing total energy by \textbf{42.8\,\%} over LAAKA and \textbf{32.4\,\%}
    over Zhou et al.
  \item Consistent gains are observed across varying network sizes
    ($N \in \{30, 50, 80, 100, 120\}$) and AS pool sizes (2 to 10).
  \item Desynchronisation recovery reduces per-device recovery energy by
    \textbf{35.3\,\%} over DAuth~\cite{das2026comsnets} with zero additional
    overhead in the recovery round.
\end{itemize}

Hardware measurements on Raspberry Pi~4B confirm the following ordering:
DAuth (0.152\,J) $<$ Proposed (0.286\,J) $<$ Zhou et al.\ (0.333\,J) $<$ LAAKA (0.602\,J).
The proposed scheme achieves energy reductions of \textbf{52.5\,\%} over LAAKA and
\textbf{14.2\,\%} over Zhou et al.\ per authentication round.

Among the four compared schemes, the proposed scheme is the only one to simultaneously
achieve decoupled distributed authentication, PUF-based secret derivation, hash-based
accumulator authentication, authentication secret update, device anonymity via
per-session pseudonym rotation, and desynchronisation recovery — all at low
computational and communication overhead suitable for resource-constrained multihop
IoT devices.
