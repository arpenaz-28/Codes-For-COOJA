#!/usr/bin/env bash
# 02-sync-project.sh — Copy this project folder to Fog and Node RPis via rsync.
# Run from the laptop.
set -euo pipefail
...
for target in "AS" "NODE"; do
  ...
  echo "[sync] Copying LAAKA folder to ${user}@${host}:${REMOTE_BASE_DIR}/${PROJECT_DIR_NAME}/"
