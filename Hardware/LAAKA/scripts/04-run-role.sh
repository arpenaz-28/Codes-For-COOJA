...
case "$ROLE" in
  gw)
    echo "[run] Starting RA (laptop)         — listens on port 5683"
    exec python3 "$NATIVE_DIR/gw_hw.py"
    ;;
  as)
    echo "[run] Starting Fog Server (RPi #1) — listens on port 5684"
    exec python3 "$NATIVE_DIR/as_hw.py"
    ;;
  node)
    echo "[run] Starting Device Node (RPi #2) — talks to Fog + RA"
