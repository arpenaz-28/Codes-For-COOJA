...
if __name__ == "__main__":
    ...
    ra_env = os.environ.copy(); ra_env.pop("USE_MIRACL", None)
    ...
    # ── Collect fog-registration (§4.2.1) one-time setup cost from Fog (Apex) ──
    try:
        fog_json = os.path.join(RESULTS_DIR, f"fogreg_{run_num:02d}.json")
        c = paramiko.SSHClient()
        c.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        c.connect(APEX_IP, username="apex", password=PASSWORD, timeout=10)
        sftp = c.open_sftp()
        sftp.get(f"/home/apex/{REMOTE_DIR}/fogreg_hw_run.json", fog_json)
        sftp.close(); c.close()
        fr = json.load(open(fog_json))
        print(f"[ORCH] Fog registration (§4.2.1, one-time): "
              f"{fr['energy_j']:.6f} J  {fr['wall_s']:.4f} s  (cpu {fr['cpu_s']:.4f} s)")
    except Exception as e:
        print(f"[ORCH] WARNING: Could not collect fog-reg JSON: {e}")
