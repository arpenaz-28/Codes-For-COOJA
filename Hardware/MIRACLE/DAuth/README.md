# DAuth — re-hosted on the Proposed scheme's transport (end-to-end, MIRACL)
...
| `compare_fair.py` | runs Proposed vs DAuth interleaved, same session |
...
python run_simulation.py 1     # one DAuth run
python compare_fair.py 6       # Proposed vs DAuth, 6 runs, same session
```
...
| DAuth | 0.1057        | 0.0278      | 15.9      | **11.9**   |

**KeyEx dropped from ~24 ms (old pull-based DAuth) to ~12 ms**, matching
Proposed. With transport equalized, DAuth is now **marginally lighter than
