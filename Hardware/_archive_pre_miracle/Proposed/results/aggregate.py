#!/usr/bin/env python3
"""
Aggregate Proposed scheme hardware runs into summary CSVs and JSON.
Reads all run_NN.json files in this directory.

Outputs:
  proposed_hw_all_rounds.csv   — per-run, per-round raw data
  proposed_hw_summary.csv      — per-run aggregate (enrollment + avg auth/round)
  proposed_hw_aggregate.csv    — cross-run mean ± 95% CI (use for paper)
  proposed_hw_aggregate.json   — same, machine-readable
"""
...
all_rounds_path = os.path.join(HERE, "proposed_hw_all_rounds.csv")
...
# ── Per-run summary CSV ────────────────────────────────────────────────────────
summary_path = os.path.join(HERE, "proposed_hw_summary.csv")
...
agg_json_path = os.path.join(HERE, "proposed_hw_aggregate.json")
with open(agg_json_path, "w") as f:
    json.dump(agg, f, indent=2)

agg_csv_path = os.path.join(HERE, "proposed_hw_aggregate.csv")
...
print(f"  _ENERGY_SUM_J['Proposed'] = {agg['ak_sum_energy_mean_j']:.4f}")
print(f"  _TIME_SUM_S['Proposed']   = {agg['ak_sum_time_mean_s']:.4f}")
