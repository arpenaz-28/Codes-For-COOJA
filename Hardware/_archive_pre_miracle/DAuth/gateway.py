...
def handle_device(conn, addr, req):
    ...
    as_sock.send(json.dumps({'type': 'key_req', 'ID_D': id_d, 'ID_AS': 'auth_server'}).encode())
...
def start():
    ...
    while True:
        ...
        try:
            ...
            elif data.get('type') == 'data_comm':
                id_d = data.get('ID_D', 'unknown')
                print(f"[Gateway] Data received from {id_d}")
                conn.send(json.dumps({'status': 'ack'}).encode())
                conn.close()
            else:
