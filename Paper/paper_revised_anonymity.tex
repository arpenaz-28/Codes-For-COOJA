...
The scheme comprises three phases: (1)~Node Enrolment Phase, (2)~Node
Authentication Phase, and (3)~Node Session Key Exchange Phase.

\subsection{Node Enrolment Phase}

Enrolment is executed once, over an end-to-end encrypted channel, after $D$
learns the address and identity $ID_{AS}$ of its assigned $AS$.
Figure~\ref{fig:enrolment} details the phase.
...
In this phase, $D$ proves group membership to $AS$ using a pseudonym,
without revealing $ID_D$.  Figure~\ref{fig:auth} illustrates the phase.
...
After successful authentication (Phase~2), $AS$ establishes a session key
...
$AS$ then simultaneously sends $(m_H, ts_2)$ to $D$ and
...
\begin{figure}[!t]
  ...
  \fbox{\parbox{0.95\columnwidth}{\scriptsize\ttfamily
    ...
    \hspace{1em}inj-event(DeviceEnrolmentStarts(id\_D, id\_AS)) is \textbf{true}.\\[2pt]
...
the mapping $\mathsf{PID}_\mathsf{old} \to K_{GW\text{-}D}$ for session continuity.
