...
\begin{enumerate}
  ...
  \item A hardware-based performance evaluation on Raspberry Pi 3B+
    ...
    byte overhead versus the base scheme~\cite{das2026comsnets}.
\end{enumerate}

Section~II surveys related work.  Section~III presents prerequisites.
Section~IV summarises the base scheme and its limitations.  Section~V
presents the proposed scheme.  Section~VI gives the security analysis.
Section~VII presents performance results.  Section~VIII concludes.
...
tracking—$D$ computes its current pseudonym:
\begin{equation}
  \mathsf{PID} = H(ID_D \| m_\mathsf{curr}).
\end{equation}
...
probability of predicting future session key material increases—as:
\begin{equation}
...
perspectives:
\begin{align*}
  &\mathsf{inj\text{-}event}(\mathsf{DeviceAuthRound1Done}(ID_D,ID_{AS}))\\
  &\quad\Rightarrow\mathsf{inj\text{-}event}(\mathsf{DeviceAuthStarts}(ID_D,ID_{AS}))
\end{align*}
...
\begin{equation*}
  \mathsf{query}\ \mathsf{attacker}(\mathsf{SecretK\_GW\_D\_Device}[])\;\Rightarrow\;\mathtt{false},
\end{equation*}
