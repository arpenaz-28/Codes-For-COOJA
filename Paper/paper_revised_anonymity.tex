...
multihop IoT networks.  The scheme decouples the node-joining (enrollment)
process from the authentication process: enrollment is performed once over an
end-to-end secure channel, while authentication and key exchange run
independently over the open channel at every session.  Gateway-selected
authentication servers (which may be parent or non-parent nodes) authenticate
newly joined devices using a two-round protocol that separates
group-membership verification from session key establishment.  Device anonymity is preserved through per-session
...
simultaneously:
\begin{itemize}
  \item \textbf{Decoupling:} The node-joining (enrollment) process and the
    authentication process are explicitly separated.  Enrollment is executed
    once over an end-to-end secure channel at network-join time; authentication
    and key exchange run independently over the open channel at every session.
    The gateway-selected authentication server may be any capable node—parent
    or non-parent—so authentication load is not skewed toward whichever node
    a device happened to join under.
...
authentication without a centralized authority, yet the scheme does not provide a multihop topology with a clean separation
between enrollment and per-session authentication.
...
\begin{table}[!t]
  ...
  \raggedright
  \scriptsize
  \textit{Dec.}: Enrollment (join) and authentication phases are explicitly separated.\ \
...
nodes/authentication servers.  The scheme \emph{decouples} the node-joining (enrollment) process from the
authentication process: enrollment is executed once over a secure channel at
network-join time, while authentication and key exchange run independently
over the open channel at each session.  The authentication server may be a
parent or a non-parent node, preventing newly joined child nodes from
overwhelming their parent with biased authentication load.
...
\begin{figure}[!t]
  ...
  \begin{tikzpicture}[
    ...
    \node[notebox, text width=3.8cm, anchor=north east] at (\xD-0.15,-3.55){
      ...
      Generate unique secret $y_D$ (random)\\
...
$D$ initiates enrolment by sending its identity $ID_D$ to $AS$.  $AS$
issues a PUF challenge $c_D$ and a session random $m_D$.  Using $c_D$, $D$
computes $R_D = \mathsf{PUF}_D(c_D)$ and generates a unique secret $y_D$
(random) and a reverse challenge $c_{AS\text{-}D}$.  It returns
$(y_D, R_D, c_{AS\text{-}D})$ to $AS$ over the secure channel.

$AS$ hashes the unique secret to obtain $Y_D^H = H(y_D)$ and folds it into
