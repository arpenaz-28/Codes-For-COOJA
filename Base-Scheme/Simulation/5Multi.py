...
for client_id, roles in auth_data.items():
    if all(role in roles for role in ['client', 'server']):
        total_energy = roles['client']['energy'] + roles['server']['energy'] 
        total_cpu = roles['client']['cpu'] + roles['server']['cpu'] 
...
if total_energy_vals and total_cpu_vals:
    avg_energy = np.sum(total_energy_vals)/20
  #  std_energy = np.std(total_energy_vals)

    avg_cpu = np.sum(total_cpu_vals)/20
