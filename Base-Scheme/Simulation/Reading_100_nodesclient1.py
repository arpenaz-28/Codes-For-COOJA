...
log_file_path = "100(Single Auth +data)1.txt"
...
total_energy_vals = []
total_cpu_vals = []
node_counts = [20]  # Replace with actual number of nodes
...
if total_energy_vals and total_cpu_vals:
    avg_energy = (np.sum(total_energy_vals)/node_counts).item()
#    std_energy = float(np.std(total_energy_vals)

    avg_cpu = (np.sum(total_cpu_vals)/node_counts).item()
   # std_cpu = np.std(total_cpu_vals)

    print(f"\n✅ Average Energy per Authentication: {avg_energy:.4f} mJ ")
    print(f"✅ Average CPU Time per Authentication: {avg_cpu:.4f} ms ")
else:
...
# Energy Plot
plt.subplot(1, 2, 1)
plt.bar(node_counts, [avg_energy], capsize=5)
...
# CPU Plot
plt.subplot(1, 2, 2)
plt.bar(node_counts, [avg_cpu], capsize=5)
plt.xlabel("Number of Nodes")
