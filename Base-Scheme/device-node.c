/* ==========================================================================
 * device-node.c  —  IoT Device (Base Scheme, unified build)
 *
 * Nodes 81-100.  Protocol:
 *   reg == 0         → Enrollment: /test/reg + /test/reg1 to AS
 *   auth == 0, cnt<1 → Auth: /test/auth to AS, compute session key
 *   auth == 1, cnt<2 → Key Exchange: /test/keyupdate to GW (DH) + data
 *   cnt >= 2         → Ongoing data to GW
 *
 * Energy measurements:
 *   ENROLL_ENERGY  — around enrollment block (reg==0)
 *   KEYEX_ENERGY   — around key-update CoAP only (auth==1, first time)
 *   AUTH_ENERGY     — around auth block (auth+session key derivation)
 * ========================================================================== */
...
PROCESS_THREAD(device_node, ev, data)
{
    ...
    while (1) {
        ...
        if (etimer_expired(&et)) {
            ...
            /* ============================================================
             * AUTHENTICATION — compute and verify session key
             * ============================================================ */
                ...
                /* Build auth payload: hash = SHA256([R_d, M_d, id_d, ts_1]) XOR Y_d_H */
                uint8_t data_c[35];
                memset(data_c, 0, 35);
                data_c[0] = R_d;
                ...
                /* Unmask received key: M_d_new = received XOR hash */
            ...
            /* ============================================================
             * KEY EXCHANGE — DH key update with GW + first data
             * ============================================================ */
            } else if (auth == 1 && count < 2) {

                /* === KEYEX BEFORE snapshot === */
                print_energest_stats(&cpu_keyex_before, &energy_keyex_before);

                /* DH key exchange: send alpha = (g^a) mod p, encrypted */
                memset(K_GW_D, 0, 16);
                for (int i = 0; i < 16; i++)
                    K_GW_D[i] = k_gw_d[i];
