...
- The **hardware** evaluation chart (`fig_hw_comparison.png`) now plots **Proposed, DAuth, LAAKA, Zhou** — DAuth added (purple `#7E5BA6`, dot hatch `...`) to `Hardware/plot_hw_comparison.py` and `Hardware/plot_hw_with_enrollment.py`. DAuth hardware data: 2 RPi 4B runs (Apex=device, Pi=AS, Laptop=GW), 1 warm-up round discarded. Auth+KeyEx 3-round sums: energy=0.532 J / time=0.140 s → **0.177 J/round** and **0.047 s/round**. Enrollment: 0.416 J / 0.110 s (avg of 2 runs). DAuth < Proposed < Zhou < LAAKA in both metrics.
