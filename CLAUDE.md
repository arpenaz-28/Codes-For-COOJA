...
- **Desync-recovery chart (`fig_desync_bar`) plots ONLY DAuth vs Proposed** (single recovery bar each, energy + CPU). **Both Zhou AND LAAKA are EXCLUDED** — each retains its previous pseudonym (Zhou's GW *adds* `DIDi_new` keeping old `DIDi`; LAAKA's fog keeps prior `TIDd`), so a missed final message self-corrects on the next round without re-registration → neither is a valid "no-recovery" baseline. Only DAuth genuinely single-states the nonce and must re-enrol; Proposed's dual-state (`m_old`,`PID_old`) is the fix → clean controlled comparison on an otherwise-identical protocol. Recovery cost: DAuth 134.8 mJ / 2.18 s, Proposed 87.2 mJ / 1.41 s → Proposed −35.3% (energy and CPU). The faithful Zhou desync sim (`Zhou-Scheme/Src-DesyncDemo-Faithful/`, runner `run_zhou_desync_faithful.py`) and its +97.1% figure are RETIRED — the desync it produced was an artifact (GW used the stale user-provided `SIDn` in β instead of the sensor's current `SIDn`); Zhou is desync-tolerant. Do NOT cite a Zhou desync penalty.
