"""
run_20_79.py
Run Revised-Anonymity (1 GW + 20 AS + 79 Devices) in COOJA locally.
...
Topology:
  ...
  Nodes 22-100 = IoT Devices (79 total)
  Assignment:  AS = 2 + ((node_id - 22) % 20)
               → 19 AS nodes serve 4 devices, 1 AS serves 3 devices

Output:
  Simulation results/20_79/logs/testlog_seed<N>.txt
  Simulation results/20_79/csv/enroll-results.csv
  Simulation results/20_79/csv/auth-results.csv
  Simulation results/20_79/csv/keyex-results.csv
  Simulation results/20_79/csv/summary.csv
"""
...
PROJECT_DIR = "/home/apex/contiki-ng/examples/cooja_20_79"
SCHEME_PATH = os.path.join(BASE, "Revised-Anonymity", "Revised-Anonymity-20_79")

SEEDS       = [123456, 234567, 345678, 456789, 567890]
NUM_DEVICES = 79

OUTPUT_DIR  = os.path.join(BASE, "Revised-Anonymity", "Simulation results", "Revised-Anonymity", "20_79", "csv")
LOG_DIR     = os.path.join(BASE, "Revised-Anonymity", "Simulation results", "Revised-Anonymity", "20_79", "logs")
...
# Early-exit COOJA script — stops the moment all NUM_DEVICES report KEYEX_ENERGY
COOJA_SCRIPT = f"""\
...
def make_csc(seed):
    """Generate COOJA .csc for 1 GW + 20 AS (2-21) + 79 Devices (22-100)."""
    # 10×10 grid — exactly 100 slots for 100 nodes
    grid = []
    for row in range(10):
        for col in range(10):
            grid.append((col * 30.0, row * 30.0))

    proj = "[CONTIKI_DIR]/examples/cooja_20_79"

    lines = []
    lines.append('<?xml version="1.0" encoding="UTF-8"?>')
    lines.append('<simconf version="2022112801">')
    lines.append('  <simulation>')
    lines.append('    <title>Revised-Anonymity 20AS+79Dev</title>')
    ...
    csc_path = os.path.join(BASE, f"_tmp_20_79_{seed}.csc")
...
def deploy_and_build():
    ...
    for src in SOURCES:
        ...
        if not os.path.exists(src_path):
            print(f"  MISSING: {src_path}"); return False
        shutil.copy2(src_path, os.path.join(PROJECT_DIR, src))
        print(f"  Copied  {src}")

    # PROJECT_DIR is 2 levels below contiki root (examples/cooja_20_79/)
    ...
    r = subprocess.run(["make", "TARGET=cooja"], cwd=PROJECT_DIR,
                       capture_output=True, text=True, timeout=300)
    if r.returncode != 0:
        print("  BUILD FAILED:"); print(r.stderr[-1200:]); return False
    cooja_bin = os.path.join(PROJECT_DIR, "build", "cooja")
    built = [f for f in os.listdir(cooja_bin) if f.endswith(".cooja")]
    print(f"  Built:  {built}")
    return len(built) >= 3


def run_sim(csc_path):
    cmd = ["./gradlew", "--no-watch-fs", "run",
           f"--args=--no-gui --contiki={CONTIKI_DIR} --autostart {csc_path}"]
    t0 = time.time()
    r = subprocess.run(cmd, cwd=COOJA_DIR, capture_output=True,
...
if __name__ == "__main__":
    print("=" * 70)
    print("Revised-Anonymity 20_79 — 5-Seed COOJA Simulation (efficient mode)")
    print(f"  Seeds: {SEEDS}  |  Early exit when all {NUM_DEVICES} devices complete")
    print("  1 GW  |  20 AS (Nodes 2–21, all active)  |  79 Devices (22–100)")
    print("=" * 70)
    ...
    for seed in SEEDS:
        ...
        if not ok:
            ...
            with open(dbg, "w") as f: f.write(raw_out[-5000:])
            print(f"  Debug → {dbg}")
        log = fetch_testlog(seed)
        if not log:
            print("  Could not retrieve testlog — skipping"); continue
    ...
    if not all_seeds:
        print("\nNo results — aborting."); sys.exit(1)
    ...
    print(f"\n{'=' * 80}")
    print("REVISED-ANONYMITY 20_79 — RESULTS (5-seed average per device)")
    print(f"{'=' * 80}")
    ...
    with open(summary_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["Phase","Seeds","Avg_CPU_s","Std_CPU_s","Avg_Energy_J","Std_Energy_J"])
        for key, name in phases:
            ...
            w.writerow([name, len(cpus), f"{ac:.6f}", f"{sc:.6f}", f"{ae:.6f}", f"{se:.6f}"])
