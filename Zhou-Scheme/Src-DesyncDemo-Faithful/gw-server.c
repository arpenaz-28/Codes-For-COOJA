        ...
        while (!PIPE_EMPTY()) {
            ...
            if (!m3_received) {
                printf("GW-S %u: Auth aborted — no M3 from sensor %u\n",
                       node_id, sn);
                /* DESYNC DEMO: notify the user of auth failure (1-byte fail M4)
                 * so it can recover immediately instead of idle-listening for a
                 * timeout. A gateway signalling auth failure is realistic and
                 * keeps the measured recovery cost = re-registration work. */
                {
                    coap_endpoint_t ep_uf;
                    uip_ipaddr_t uf_addr;
                    uint8_t uf = p->user_node_id;
                    uip_ip6addr_u8(&uf_addr, 0xfd,0,0,0,0,0,0,0,
                                   0x02,uf,0,uf,0,uf,0,uf);
                    uip_ipaddr_copy(&ep_uf.ipaddr, &uf_addr);
                    ep_uf.port = UIP_HTONS(COAP_DEFAULT_PORT);
                    uint8_t fail = 0xFF;
                    coap_init_message(req_out, COAP_TYPE_CON, COAP_POST, coap_get_mid());
                    coap_set_header_uri_path(req_out, "test/auth_complete");
                    coap_set_payload(req_out, &fail, 1);
                    COAP_BLOCKING_REQUEST(&ep_uf, req_out, m4_delivery_handler);
                }
                p->valid = 0;
                pipe_head = (pipe_head + 1) % PIPELINE_SIZE;
                continue;
